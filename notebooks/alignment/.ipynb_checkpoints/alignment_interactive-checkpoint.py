...
@dataclass
class SuspicionFlag:
    kind: str  # 'count_mismatch' | 'position' | 'ratio'
    description: str
...
def span_extend_left(full_text: str, span: MatchSpan) -> MatchSpan:
    ...
    # include any whitespace between new word and old start
...
def span_shrink_left(full_text: str, span: MatchSpan) -> MatchSpan:
    ...
    # new start = position after first token + following whitespace
...
def span_shrink_right(full_text: str, span: MatchSpan) -> MatchSpan:
    ...
    # new end = just before last token (strip trailing whitespace)
...
def process_dataframe(
    ...
    for idx, row in filtered.iterrows():
        ...
        # skip rows where BOTH sides have no matches
        if not akk_matches and not eng_matches:
            continue
...
def split_with_exhaustion(
    ...
    Example
    -------
    akk: [pre] M(aaa) [bbb remaining]
    eng: [pre] M(aaa) [nothing left]   <- exhausted after first match

    result: [("pre","pre"), ("aaa bbb", "aaa")]
             the "bbb" is absorbed into the last akk match text
    """
    ...
    # if either side has no active matches, return as a single unsplit pair
    ...
    for akk_m, eng_m in zip(active_akk, active_eng):
        ...
        if akk_exhausted or eng_exhausted:
            # Option B: absorb all remaining text into this last match
    ...
    # both sides still have text after all matches — append the tails
...
class BatchReviewer:
    ...
    def _render_controls(self, r: DatapointResult):
        ...
        def make_match_row(
            ...
            # also refresh text when skip checkbox changes
            skip_cb.observe(
        ...
        def _on_apply(_=None):
            ...
            r.approved = True
            print(
                f"[APPLY] row {r.row_index} | approved={r.approved} | discard={r.discard}"
            )
            print(f"  akk matches saved: {len(r.custom_akk_matches)}")
            for i, m in enumerate(r.custom_akk_matches):
                print(f"    [{i}] {m.start}-{m.end}: {m.text[:60]}")
            print(f"  eng matches saved: {len(r.custom_eng_matches)}")
            for i, m in enumerate(r.custom_eng_matches):
                print(f"    [{i}] {m.start}-{m.end}: {m.text[:60]}")
            self._render()
        ...
        children.append(widgets.HBox([apply_btn, remove_btn]))
